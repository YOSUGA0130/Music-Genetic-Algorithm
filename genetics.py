...
# 遗传算法概率参数
CROSSOVER_PROBABILITY = 0.1             # 交叉概率
MUTATION_PROBABILITY = 0.1              # 变异概率
TRANSPOSITION_PROBABILITY = 0.05        # 移调概率
RETROGRADE_PROBABILITY = 0.05           # 逆行概率
INVERSION_PROBABILITY = 0.05            # 倒影概率
RETROGRADE_INVERSION_PROBABILITY = 0.03 # 逆行倒影概率
...
def evolve(population: List[List[int]],
           ...
           retrograde_inversion_probability = RETROGRADE_INVERSION_PROBABILITY) -> List[List[int]]:
    """
    进化生成下一代种群
    ...
    return next_generation[:n]


def roulette_wheel_selection(n, fitness_values: List[float]) -> int:
    """
    ...
    # 转换为 numpy 数组以进行向量化操作
    fitness_values = np.array(fitness_values)
    
    # 确保适应度非负 (防止负数导致概率计算错误)
